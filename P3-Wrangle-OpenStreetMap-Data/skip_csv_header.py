...
def add_unique_id(filename, newfilename,idname):
    """adds a unique id as a column to the original csv file and return a new csv file"""
...
def skip_header(filename,newfilename):
    """remove the header line of csv file and return a new csv file without the header"""
    with open(filename, "rb") as infile, open(newfilename, "wb") as outfile:
        reader = csv.reader(infile)
